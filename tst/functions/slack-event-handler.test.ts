...
import { ChatSyncCommandOutput, ChatCommandOutput } from '@aws-sdk/client-qbusiness';
...
describe('Slack event handler test', () => {
  ...
  test('Should chat with context simple text', async () => {
    const response = await handler(
      ...
      {
        ...MOCK_DEPENDENCIES,
        callClient: () => Promise.resolve(amazonQValidResponse2TextSimple as ChatCommandOutput),
        getItem: async () =>
          Promise.resolve({
            Item: {
              conversationId: 'conversationId',
