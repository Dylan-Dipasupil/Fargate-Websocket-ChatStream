...
export class MyAmazonQSlackBotStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: cdk.StackProps, env: StackEnvironment) {
    ...
    const initialSlackSecretContent = JSON.stringify({
      ...
      SlackAppToken: '<Replace with App Token>'
    });
    ...
    const slackLambdaExecutionRole = new Role(this, `${props.stackName}-SlackLambdaExecutionRole`, {
      ...
      inlinePolicies: {
        ...
        ChatPolicy: new PolicyDocument({
          statements: [
            new PolicyStatement({
              actions: ['qbusiness:ChatSync', 'qbusiness:PutFeedback', 'qbusiness:Chat'],
    ...
    const fargateTaskRole = new Role(this, `${props.stackName}-FargateTaskRole`, {
      ...
      inlinePolicies: {
        ...
        QbusinessPolicy: new PolicyDocument({
          statements: [
            new PolicyStatement({
              actions: ['qbusiness:ChatSync', 'qbusiness:PutFeedback', 'qbusiness:Chat'],
              resources: ['arn:aws:qbusiness:*:*:application/*'] //
    ...
    const ecrRepository = new ecr.Repository(this, 'MyEcrRepo', {
      repositoryName: 'slack-bolt-app',
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });

    const dockerImageAsset = new ecr_assets.DockerImageAsset(this, 'SlackBoltImage', {
      directory: ".",
    });
    ...
    const logGroup = new logs.LogGroup(this, 'SlackBoltLogGroup', {
      logGroupName:  `${props.stackName}-SlackBoltApp`,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });


    const container = taskDefinition.addContainer('SlackBoltContainer', {
      ...
      logging: ecs.LogDriver.awsLogs({ 
        streamPrefix: `${props.stackName}-SlackBoltApp`,
        logGroup: logGroup
      })
    });
    ...
    [
      ...
      {
        handler: 'ws-app-handler',
        id: 'WSAppHandler',
        description: 'Handler for WebSocket connections'
      }
    ].map((p) => {
