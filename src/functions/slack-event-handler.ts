...
import { ERROR_MSG, getMarkdownBlock, getSlackSecret, updateSlackMessage, validateSlackRequest } from '@helpers/slack/slack-helpers';
...
export const handler = async (
  ...
  const [output, slackMessage] = await Promise.all([
    chat( prompt, attachments, dependencies, slackEventsEnv, iamSessionCreds, body.event.user, context),
...
const FLUSH_INTERVAL = 500; // x seconds in milliseconds
const CHAR_THRESHOLD = 150; // y characters
...
const slackClient = new WebClient(secret.SlackBotUserOAuthToken);
...
const flushBuffer = () => {
  if (buffer) {
    ...
    slackClient.chat.update({
      ...
      ts: slackMessage.ts ?? '',
      text: outputData.outputText,
      blocks: getResponseAsBlocks(outputData.outputText, latestTextEvent?.systemMessageId ?? '')
    });
    lastFlushTime = Date.now();
  }
  setTimeout(flushBuffer, FLUSH_INTERVAL);
};

const shouldFlush = () => {
  const now = Date.now();
  return buffer.length >= CHAR_THRESHOLD || (now - lastFlushTime) >= FLUSH_INTERVAL;
};

latencyArray.push(Date.now() - startTime);
...
await Promise.all([
  ...
  updateSlackMessage(slackEventsEnv, slackMessage, outputData.outputText, [...contentBlocks])
]);
...
logger.debug(`Latency Analysis: ${JSON.stringify(latencyArray)}`);
